...
if not _HAS_PYTEST:
    ...
    class TestRealizedReconciliationDependencies(unittest.TestCase):
        @unittest.skip("Requires pytest for realized reconciliation tests")
    ...
    def test_reconcile_close_first_backfills_terminal_sell_completions_when_no_pending(temp_tools):
        tools = temp_tools
        tools.db.execute(
            """
            INSERT INTO agent_trades (agent, pair, side, price, quantity, total_usd, order_type, order_id, status, pnl)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            ("test_agent", "ETH-USD", "SELL", 100.0, 1.0, 100.0, "limit", "ord-sell-terminal-1", "filled", 2.0),
        )
        tools.db.commit()

        summary = rat.reconcile_close_first(tools, max_orders=10, lookback_hours=48)
        assert summary["close_attempts"] >= 1
        assert summary["close_completions"] >= 1
        assert summary["close_gate_passed"] is True
        assert summary["close_gate_reason"] == "sell_close_completion_observed"
